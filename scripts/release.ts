...
if (args.includes("-h") || args.includes("--help")) {
  console.log();
  console.log(`  ${bold("release")} ${dim("— tag & publish a new Epist version")}`);
  console.log();
...
async function main() {
  ...
  // 7. GitHub release (draft for RCs, published for stable)
  const ghArgs = isRC
  ...
  console.log();
  console.log(`  ${dim("View:")} ${cyan(`https://github.com/nick-skriabin/epist/releases/tag/${tag}`)}`);
  console.log();
}
